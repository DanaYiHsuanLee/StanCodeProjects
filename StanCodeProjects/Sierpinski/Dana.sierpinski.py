...
def sierpinski_triangle(order, length, upper_left_x, upper_left_y):
	"""
	:param order:int,  recursive depth of  sierpinski triangle
	...
	if order == 0:
		...
		tri.add_vertex((upper_left_x + length / 2, upper_left_y + (length ** 2 - (length / 2) ** 2) ** 0.5))
		window.add(tri)
		pause(200)
		sierpinski_triangle(order-1, length/2, upper_left_x, upper_left_y)
		sierpinski_triangle(order - 1, length / 2, upper_left_x+length/2, upper_left_y)
		sierpinski_triangle(order - 1, length / 2, upper_left_x+length/4, upper_left_y+((1/2*length)**2-(1/4*length)**2)**0.5)
